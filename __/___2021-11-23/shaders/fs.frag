...
void main() {
  ...
  // color += s * (vec3(0., 59., 70.) / 255.) +
  //          (1. - s) * (vec3(102., 165., 173.) / 255.);
  ...
  //     0.3, 0.3 + fw,
  //     length(vec2(x, y) - vec2(0.5 + sin(20. * time + vUv.y * 17.) * 0.07,
  //                              0.5 - cos(20. * time + vUv.x) * 0.03)));
  // color =
  //     mix(color, vec3(vUv.x + sin(time), vUv.y + sin(time), sin(time
  //     + 1.57)),
  //         shape);
  // // diffuseColor.rgb = vec3(1.0, 0., 0.);
